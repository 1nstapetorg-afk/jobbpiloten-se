...
const TEXT_MODELS = {
  // Round-95 (2026-08-07): secondary swapped from llama-4-maverick
  // (returns 404 "does not exist" on this account) to
  // openai/gpt-oss-120b — live-verified via the Groq /models + a
  // chat-completions probe. gpt-oss is a reasoning model; the
  // `reasoning_effort: 'low'` pin lives in createChatWithFallback so
  // the 180-400 token budgets of the text generators aren't eaten by
  // the internal reasoning pass.
...
async function createChatWithFallback(params) {
  ...
  for (const model of models) {
    try {
      ...
      // Round-95: gpt-oss models on Groq are reasoning models — they
      // burn output tokens on an internal reasoning pass BEFORE the
      // answer (live probe: ~164 tokens at default effort vs ~25 with
      // 'low'). The generators budget 180-400 max_tokens, so default
      // reasoning would truncate the letter/email. Pin it low ONLY
      // for gpt-oss; other chain models (qwen) reject the param.
      ...
      // Round-94 followup (2026-08-07): a PROMPT_ECHO (the LLM
      // reproduced the prompt instead of generating) is ALSO a
      // retryable signal — the primary model produced no usable
      // content, so burn the chain like a model-level rejection and
      // let the secondary model take a shot. Without this, a
      // prompt-echoing primary (observed live: qwen/qwen3.6-27b)
      // 503'd every request even though llama-4-maverick was sitting
      // in the chain. If EVERY model echoes, the last PROMPT_ECHO
      // propagates and callers still map it to the retryable 503.
      const isEcho =
        err?.code === PROMPT_ECHO_ERROR_CODE || err?.error === PROMPT_ECHO_ERROR_CODE
      if ((isModelLevelError(msg) || isEcho) && models.length > 1) {
        console.warn(`[groq] text model ${model} ${isEcho ? 'echoed the prompt' : `rejected (${msg})`} — trying next in chain`)
        continue
      }
  ...
  // All models rejected (model-level error OR prompt echo) — propagate
  // the last error so the caller's catch runs the rule-based fallback.
  throw lastErr
}
...
/**
 * Round-88 — admin health probe for /api/admin/ai-status.
 *
 * One minimal 1-token LLM call to classify provider health WITHOUT
 * leaking any secret:
 *   • mock mode (Round-87 SKIP_LLM_E2E / CI gate) → reported as
 *     mockMode:true, status ok — the operator must not mistake the
 *     deterministic E2E mock for a quota outage.
 *   • no provider key configured → degraded / not-configured.
 *   • a 429 whose message matches the Groq TPD-exhaustion class
 *     (Round-86 isTpdQuotaError) → quotaExhausted:true — the signal
 *     that every AI surface is soft-failing to fallbacks until the
 *     daily reset / tier upgrade.
 *   • any other failure → degraded / unreachable.
 *
 * NEVER includes the API key or any request payload in the result.
 * The response shape is deliberately small so a dashboard/admin UI
 * can render it without parsing provider internals.
 */
export async function probeGroqHealth() {
  if (isLlmMockMode()) {
    return { status: 'ok', mockMode: true, quotaExhausted: false, reachable: true, detail: 'mock-mode' }
  }
  if (!isLlmAvailable() || !client) {
    return { status: 'degraded', mockMode: false, quotaExhausted: false, reachable: false, detail: 'not-configured' }
  }
  try {
    ...
    const content = response?.choices?.[0]?.message?.content
    if (content && isPromptEcho(content)) {
      return { status: 'degraded', mockMode: false, quotaExhausted: false, reachable: true, detail: 'echo' }
    }
    return { status: 'ok', mockMode: false, quotaExhausted: false, reachable: true, detail: 'ok' }
  } catch (err) {
    const msg = String(err?.message || err || '')
    const quota = isTpdQuotaError(msg)
    return {
      status: 'degraded',
      mockMode: false,
      quotaExhausted: quota,
      reachable: false,
      detail: quota ? 'quota-exhausted' : isModelLevelError(msg) ? 'model-level' : 'unreachable',
    }
  }
}
