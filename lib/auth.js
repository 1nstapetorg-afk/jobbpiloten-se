...
async function resolveAuthState(request) {
  ...
  if (configured) {
    ...
    // Round-85 fix (2026-08-04, "onboarding shows Unauthorized"): when
    // Clerk IS configured but the request has no Clerk session, fall
    // back to the demo cookie (x-demo-user-id header / demoUserId
    // cookie) so the onboarding wizard, sign-in demo button, and e2e
    // fixtures keep working in Clerk-configured dev environments. The
    // onboarding page sets the demo cookie before POSTing /api/profile
    // and /api/email-preview; pre-fix, Clerk mode ignored it and every
    // call 401'd ("Kunde inte spara profilen: Unauthorized" /
    // "Förhandsvisa AI-mejl → Unauthorized").
    //
    // NON-PRODUCTION ONLY: in production a real Clerk session is
    // required — the demo cookie is a dev/test affordance, not an
    // auth boundary (see lib/clerk-config.js "NOT safe for
    // production" caveat). Clerk sessions always win when present.
    if (process.env.NODE_ENV !== 'production') {
      const demoId = getDemoUserId(request)
      if (demoId) return { userId: demoId, demo: true }
    }
