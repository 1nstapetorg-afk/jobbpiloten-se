...
function isClerkKeyReal(key) {
  ...
  // Round-fix: reject template placeholders. .env/.env.template ship
  // keys as `__YOUR_CLERK_SECRET_KEY__`-style values, which are >20
  // chars and contain no `xxx` — so the earlier checks classified them
  // as "real", made isClerkConfiguredServer() return true, and drove
  // every request down the Clerk path ("Publishable key not valid" +
  // `[middleware] Clerk middleware execution failed` on every request)
  // instead of clean demo mode. Any `__...__` sentinel is unambiguous: a
  // real Clerk key never contains a double-underscore placeholder.
