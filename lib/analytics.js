...
//
// Fire-and-forget, but ALWAYS returns a resolved Promise (never
// undefined) so callers may chain `.catch()` for observability
// (Round-86 / Bug 1: /api/email-preview + /api/extension/email-body
// chained trackEvent(...).catch(...) — pre-fix the sync function
// returned undefined and `.catch` on undefined threw a TypeError
// inside the AI-generation path). Errors are swallowed so an
// analytics hiccup never blocks a user-facing action.
export function trackEvent(name, props = {}) {
  if (ANALYTICS_DISABLED) return Promise.resolve()
  ...
  if (!/^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)*$/.test(name)) return Promise.resolve()
  const safeProps = {}
  ...
  // Round-86 / Bug 1: explicit resolved-Promise return so the
  // `trackEvent(...).catch(...)` chains in the email routes never
...
// ---- trackPlausible() ----
//
// Round-89 — Plausible custom-event bridge (client-side only). The
// Plausible script tag in app/layout.js <head> auto-tracks pageviews;
// this helper forwards named custom events through
// `window.plausible('event', { props })` when the script is loaded.
// No-op when Plausible isn't present (dev, script blocked, no
// network), so analytics can never break a user-facing action, and it
// fires nothing on the server (typeof window guard) — call sites are
// client components (onboarding, dashboard, landing).
