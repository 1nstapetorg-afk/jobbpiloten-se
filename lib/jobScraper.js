 ...
 *   2. queries Ledigajobb.se (soft-block-tolerant HTML scrape — see
 *      lib/scrapers/ledigajobb.js),
 *   3. queries Jobbland.se (soft-block-tolerant HTML scrape — see
 *      lib/scrapers/jobbland.js),
 *   4. dedupes by URL then by (title + company + location).
 ...
 * `source` badge.
 *
 * 2026-08-07 (Round-94/95 followup): Blocket Jobb was REMOVED from the
 * waterfall — Schibsted shut the platform down permanently on
 * 2026-12-16 (domain jobb.blocket.se returns NXDOMAIN in public DNS;
 * see the dev-log audit that found `[blocket] fetch failed` on every
 * search). Round-95 added Jobbland.se (Duunitori, which also absorbed
 * Jobbsafari) as the third leg. The Blocket scraper module was
 * removed outright in the Round-94/95 cleanup — the platform is shut
 * down (jobb.blocket.se is NXDOMAIN), so neither its scraper nor its
 * URL builder had any remaining production caller.
 *
 * Jobbsafari and ledigajobb.se also serve as honest pre-filled URL
 * fallbacks (lib/scrapers/urlBuilders.js) for the "Letar du bredare?"
 * card.
 */

import { scrapeLedigajobbJobs } from './scrapers/ledigajobb.js'
import { scrapeJobblandJobs, buildJobblandSearchUrl } from './scrapers/jobbland.js'
import {
...
// Re-export the public URL helpers so the dashboard + apply-now route
// only have to import from '@/lib/jobScraper' (a single fan-in point).
// The retired Blocket builder (`buildBlocketSearchUrl`) and the `toSlug`
// helper were removed in the Round-94/95 cleanup — no production caller
// remained after Blocket Jobb shut down.
export {
  buildJobSafariSearchUrl,
  buildLedigaJobbSearchUrl,
  buildJobblandSearchUrl,
}
...
/**
 * Multi-source waterfall — combines AF + Ledigajobb + Jobbland,
 * dedupes, and returns a single ranked list. Used by
 * /api/jobs-available and /api/cron so both surfaces show the larger
 * combined pool.
 *
 * Dedup key order:
 *   1. URL (exact, case-insensitive trimmed) — strongest signal that
 *      two sources are listing the same ad.
 *   2. `company|title|location` — catches the case where Ledigajobb and
 *      AF both index the same posting but Ledigajobb's URL is just the
 ...
 * Ledigajobb / Jobbland jobs (and vice-versa) rather than an empty
 * array — a partial result is more useful than none.
 */
export async function multiSourceSearchJobs({ query = '', location = '', region = '', limit = 20, offset = 0, employmentTypes = null } = {}) {
  ...
  // Ledigajobb.se secondary — soft-block-tolerant (gracefully degrades
  // to [] on a 403/429 so callers see no behavioural disruption).
  // Round-94 followup: Blocket Jobb is shut down (NXDOMAIN) and no
  // longer participates — see the header comment. Operators can
  // disable this source per-env by exporting
  // LEDIGAJOBB_SCRAPER_ENABLED=false in the runtime — the route then
  // drops the leg entirely.
  ...
  // Jobbland.se tertiary — Round-95 (replaces the retired Blocket leg;
  // see the header comment). Same soft-block-tolerant contract as
  // Ledigajobb; operators can disable per-env with
  // JOBBLAND_SCRAPER_ENABLED=false.
  if (process.env.JOBBLAND_SCRAPER_ENABLED !== 'false') {
    tasks.push(
      scrapeJobblandJobs({ query, location, limit }).catch((e) => {
        console.warn('[multiSource] Jobbland scraper failed:', e?.message)
        return []
      }),
    )
  }
  const [afJobs, ledigajobbJobs, jobblandJobs] = await Promise.all(tasks)
  const ledigajobbJobsSafe = ledigajobbJobs || []
  const jobblandJobsSafe = jobblandJobs || []
  const deduped = dedupeJobs([...afJobs, ...ledigajobbJobsSafe, ...jobblandJobsSafe])
  // Apply the multi-select employment-type filter AFTER dedupe so
  // duplicate AF/Ledigajobb entries that the user has explicitly
  ...
  const upstreamCapped =
    ...
    jobblandJobsSafe.length >= limit
  const hasMore = upstreamCapped || combined.length > offset + limit
  const result = windowed

  // ---- Lightweight AF/Ledigajobb/Jobbland hit-rate metric ----
  ...
  //   af, blk, lj, jl — per-source RAW hit counts (pre-dedupe)
  //                     (blk is retired Blocket — always 0)
  //   in          — in = af + blk + lj + jl (pre-dedupe total)
  ...
  //
  // Schema versioning: `v: 2` (Round-95 added the `jl` field; bump so
  // log parsers keyed on the old field set don't misparse).
  const metric = {
    evt: 'multiSource.metric',
    v: 2,
    af: afJobs.length,
    // Round-94 followup: Blocket Jobb shut down 2026-12-16 (NXDOMAIN)
    // — the field is retained at 0 for log-shape stability.
    blk: 0,
    lj: ledigajobbJobsSafe.length,
    // Round-95: Jobbland.se (Duunitori) replaces the retired Blocket leg.
    jl: jobblandJobsSafe.length,
    in: afJobs.length + ledigajobbJobsSafe.length + jobblandJobsSafe.length,
  ...
  if (afJobs.length === 0 && ledigajobbJobsSafe.length === 0 && jobblandJobsSafe.length === 0) {
    // Ops visibility for a fully-empty scrape (AF + Ledigajobb +
    // Jobbland all returned zero jobs for this query + location). The
    // structured metric line above already carries per-source counts +
    // truncated query/location for grep-able source-health monitoring,
    // so this human-readable warning is a plain tag that operators can
    // alert on without parsing JSON.
    console.warn('[multiSource] all sources returned empty')
 ...
 * Ledigajobb / Jobbland (broader, secondary).
 */
 ...
 * everything).
 *
 * Operates on the result of `mapAFJob`/`scrapeLedigajobbJobs` — i.e.
 * each job has a normalized
