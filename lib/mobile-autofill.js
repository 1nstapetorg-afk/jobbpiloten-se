...
export function generateAutofillScript(profileData = {}, jobData = {}) {
  ...
  return '(function () {\n' +
    ...
    '    if (v === "") { return false; }\n' +
    '    if (key === "email") { return /@.+\\./.test(v); }\n' +
    '    if (key === "phone") { return /[0-9+\\-() .]{6,25}/.test(v); }\n' +
    '    if (key === "firstName" || key === "lastName" || key === "fullName") { return v.length < 200 && v.indexOf(String.fromCharCode(10)) === -1; }\n' +
    '    if (key === "salaryExpectation" || key === "payExpectation") { return !isNaN(parseFloat(v)); }\n' +
    '    return true;\n' +
    '  }\n' +
'  function fillAll() {\n' +
    ...
    '      if (!isValidValue(entry.key, value)) {\n' +
'        try { console.log("[Autofill] Skip (bad value)", entry.key, value); } catch (_e) {}\n' +
'        continue;\n' +
'      }\n' +
'      try { console.log("[Autofill] Mapping", entry.key, value); } catch (_e) {}\n' +
