...
//
// RATE-LIMIT REALITY (2026-08-02, observed live): OCR shares the SAME
// Groq token budget as cover-letter / email / answer generation — one
// org-wide daily tier (e.g. on_demand 200 000 TPD). A burst of CV
// uploads + letter generations in one day can exhaust it; the provider
// then returns 429 "Rate limit reached ... on tokens per day (TPD)"
// for EVERY OCR call until reset (~24h). This is handled by design:
// 429 is NOT a model-level error, so ocrImageBuffer returns '' without
// burning the vision-model fallback chain, and the upload falls back
// to the manual-summary UX. Operators diagnosing "OCR stopped working"
// should check the Groq usage dashboard BEFORE assuming a model
// regression — a 429 line in server logs is the tell.
