 ...
 *      `lib/scrapers/jobbland.js`'s 60s/10s cache + soft-block handling.
 *
 ...
 *     posture as Jobbland. If 403/429/network → return [], negative-cache
 *     the failure for 10s, and let the multiSource waterfall do its
 *     thing (it always has AF + Jobbland to fall back on).
...
// fingerprint the Jobbland scraper uses so the two cannot be told apart
...
// Same 60s positive / 10s negative cache windows as `jobbland.js`. The
// keys are kept isolated so a Jobbland 403 can never poison the
 ...
 *
 * URL pattern (2026-08-07 audit): `https://ledigajobb.se/sok?q=<query>
 * &l=<location>` — the site's own search form (`action="/sok"
 * method="get"` on ledigajobb.se) uses QUERY-STRING params, NOT the
 * path-segment scheme (`/sok/q-…/l-…/`) this builder emitted before.
 * The old path shape now returns 404 for every search, which is why
 * the Ledigajobb waterfall leg produced zero jobs (see the 2026-08-07
 * dev-log audit: `[ledigajobb] 404 Not Found`). Verified live: the
 * query-string form returns 200 and `l=` actually filters results.
 * Mirrors the Jobbsafari builder (lib/scrapers/urlBuilders.js) so all
 * three deep-link helpers share the same query-param convention.
 * URLSearchParams percent-encodes Swedish diacritics automatically
 * (Malmö → Malm%C3%B6), which the site accepts.
 */
export function buildLedigaJobbSearchUrl({ query = '', location = '' } = {}) {
  ...
  return `${LEDIGAJOBB_BASE}/sok?${params.toString()}`
}

/**
 * Search ledigajobb.se for matching listings. Mirrors
 * `lib/scrapers/jobbland.js`'s `scrapeJobblandJobs` signature so the
...
function resolveAndHash(rawHref) {
  ...
  // Round-96 (Bug 1): a captured href that normalises to the bare
  // homepage (path `/` or empty — e.g. `/`, `https://ledigajobb.se`,
  // `/sok?...` when the site links the whole card to its landing page)
  // is NOT a usable job URL and would take the user to the front page
  // instead of the ad. Return null so the caller discards the listing.
  // Real job links carry a `/jobb/<slug>` path, so a URL without a
  // non-root path is a bad scrape, not a deep link.
  if (abs) {
    try {
      const u = new URL(abs)
      const p = u.pathname.replace(/\/$/, '')
      if (!p || p === '/' || p === '/sok') return null
    } catch (_) {
      return null
    }
  }
...
function mapArticleListing(m) {
  ...
  const resolved = resolveAndHash(rawHref)
  if (!resolved) return null
  const { url, idHash } = resolved
  return {
