...
//
// Canonical registry for the rare application-form questions the
// extension detects dynamically at apply-time (extension/content.js
// TIER3_KEYWORDS — the regexes live there; the id+label pair lives
// here so the API route can validate `rareFields` payloads without
// importing the content script). A rare field's answer is stored on
// the profile under `rareFields: { [id]: answer }` (free-text string)
// and autofilled on subsequent pages instead of prompting again.
//
// The labels MUST stay byte-identical to the `label` values in
// extension/content.js TIER3_KEYWORDS — the content script matches
// host inputs by label keyword, and the popup saves answers keyed by
// the id below. tests/unit/extension-taxonomy-parity.test.mjs locks
// the bundled copy to this registry.
export const RARE_FIELDS = [
...
export const RARE_FIELD_IDS = RARE_FIELDS.map((f) => f.id)

export const RARE_FIELD_LABELS = Object.fromEntries(RARE_FIELDS.map((f) => [f.id, f.label]))

// ---- Round-84 — rareFields validation/coercion helper (API routes) ----
//
// Returns a cleaned `{ [id]: answer }` object. Unknown ids are
// dropped; answers must be non-empty strings capped at 500 chars.
// The profile-update + POST /api/profile handlers both use this so
// a malformed payload can never write junk into Mongo.
export function sanitizeRareFields(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {}
  const out = {}
  for (const [id, value] of Object.entries(raw)) {
    if (!RARE_FIELD_IDS.includes(id)) continue
    if (typeof value !== 'string') continue
    const trimmed = value.trim()
    if (!trimmed) continue
    out[id] = trimmed.slice(0, 500)
  }
  return out
}
