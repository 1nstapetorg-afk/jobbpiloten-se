 ...
 * privately inside lib/scrapers/blocket.js and lib/jobScraper.js (both
 * since removed); hoisting it here means the test suite can lock the
 * contract (deterministic, base36-only output) and the id-derivation
 * logic can't drift between callers (ledigajobb.js + jobbland.js today).
 *
 ...
 * collapse (see `mapArticleListing` in ledigajobb.js and `mapJobblandCard`
 * in jobbland.js), not security.
 *
