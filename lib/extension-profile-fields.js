...
//     checkbox alone)
// ---- 2026-08-03 (Round-81) — Industry-specific boolean keys ----
//
// Imported from lib/field-taxonomy.js (the shared industry registry
// consumed by onboarding / settings / the API route / the extension
// profile). Kept OUT of ROUND12_BOOLEAN_KEYS so the Round-12 count and
// its unit tests stay stable; the extension's boolean dispatch treats
// them identically (kind: 'boolean' FIELD_PATTERNS entries).
import { INDUSTRY_BOOLEAN_KEYS, INDUSTRY_IDS } from './field-taxonomy.js'

export const ALL_BOOLEAN_KEYS = [...ROUND12_BOOLEAN_KEYS, ...INDUSTRY_BOOLEAN_KEYS]

// The `industry` profile key (9 canonical ids) — kept here so the
// profile builder and the API route share one import.
export { INDUSTRY_BOOLEAN_KEYS, INDUSTRY_IDS }
...
export function getRound12Defaults() {
  ...
  // Boolean defaults — all Round-12 + industry-specific keys (incl.
  // autoConsent) → false so the extension leaves every host field
  // untouched unless the user explicitly opts in.
  for (const k of ROUND12_BOOLEAN_KEYS) out[k] = false
  for (const k of INDUSTRY_BOOLEAN_KEYS) out[k] = false
...
// ---- 2026-08-03 (Round-81) — total INDUSTRY extension fields ----
//
// The extension profile additionally exposes `industry` (string) plus
// every INDUSTRY_BOOLEAN_KEYS boolean. Derived at module load so
// tests/unit/extension-profile.test.mjs can assert the exact runtime
// key count without a hand-maintained fixture.
export const INDUSTRY_TOTAL_FIELDS = 1 + INDUSTRY_BOOLEAN_KEYS.length // +1 for `industry`