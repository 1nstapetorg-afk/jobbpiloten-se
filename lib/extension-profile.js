...
// Round-83 — complete structured industry taxonomy. sanitizeIndustryFields
// re-validates the stored industryFields object against the canonical
// field defs so a legacy/wrong-shape Mongo doc degrades to {} (never
// leaks junk into the extension).
import { sanitizeIndustryFields, sanitizeRareFields } from './field-taxonomy.js'
...
export function buildExtensionProfile(profile, latestApplication) {
  ...
  //     profile.address as a legacy fallback) still work, AND
  //     so forms with a single generic "Address" field get the
  //     FULL raw string the user typed (street-only would break
  //     a CV PDF dump into a single address field).
  //   • `address` keeps the full multi-part raw input so
  //     CV PDF reports and content.js's `profileKey: 'address'`
  //     matcher still see the original blob. The per-component
  //     routes (`street`/`zip`/`city`/`country`) serve the split
  //     fields (Postnummer / Ort / Land dedicated inputs).
  ...
  return {
    // 2026-08-03 (Round-81) — the user's industry (one of the 9
    // canonical ids in lib/field-taxonomy.js). The extension's
    // popup + content script use it to decide which industry-core
    // fields are relevant on a given page. Falls back to '' when
    // the Mongo doc has no industry (extension keeps running with
    // universal fields only).
    industry: INDUSTRY_IDS.includes(profile?.industry) ? profile.industry : '',
    // 2026-08-03 (Round-83) — complete structured industry answers,
    // nested per industry ({ lager: { forklift_license: 'Ja', … } })
    // matching the stored Mongo shape. Re-validated against the
    // taxonomy defs so only canonical field ids + option-list values
    // survive (malformed Mongo docs degrade to {}). The extension's
    // targeted fill pass + the popup read this for the effective
    // industry.
    industryFields: INDUSTRY_IDS.includes(profile?.industry)
      ? { [profile.industry]: sanitizeIndustryFields(profile.industry, profile?.industryFields?.[profile.industry]) }
      : {},
    // Round-84 — Tier-3 rare-field answers ({ [rareFieldId]: string },
    // keyed by the canonical ids in lib/field-taxonomy.js RARE_FIELDS).
    // Saved from the popup's Tier-3 prompt; the extension's
    // fillRareFields pass matches them by label and fills the input
    // directly instead of prompting again. Re-validated so a
    // legacy/wrong-shape Mongo doc degrades to {}.
    rareFields: sanitizeRareFields(profile?.rareFields),
    ...
    email: profile?.email || '',
    phone: profile?.phone || '',
    // 2026-07-21 (BUG 3 + Storgatan-regression fix) — expose
    // per-component address keys for split-field forms (Swedish
    // ATS sites with Postnummer / Ort / Land dedicated inputs)
    // AND keep `address` as the full raw multi-part string for
    // generic single-field forms + CV PDF reports. The previous
    // shape (`address: addressParts.street || address`) truncated
    // generic forms incorrectly — restored to legacy behaviour.
    // `city` was already a legacy key (kept); `zip` is deduplicated
    // from a previously-duplicate key declaration so the parsed
    // value flows through instead of an empty re-declaration
    // overwriting it.
    address,
    ...
    // ---- 2026-08-03 (Round-81) — Industry-specific booleans ----
    // Mirrors the ROUND12_BOOLEAN_KEYS coercion: strict Boolean() so
    // the JSON output is always true | false. These feed the new
    // industry FIELD_PATTERNS entries in extension/content.js
    // (truckkörkort/fysiskt arbete/skift for lager, vårdbiträde/HLR
    // for vård, CE-körkort for transport, …).
    ...Object.fromEntries(
